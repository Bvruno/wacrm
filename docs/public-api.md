...
The public API lets you drive your CodixIA instance from your own
...
3. Copy the key. **The full key is shown exactly once.** CodixIA
...
Rather than polling, register an endpoint and CodixIA will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; CodixIA keeps only an encrypted copy.
...
Headers: `X-CodixIA-Event`, `X-CodixIA-Webhook-Id`, and `X-CodixIA-Signature`.

### Verifying the signature

`X-CodixIA-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages CodixIA stores (inbox + API sends), not broadcast-only
