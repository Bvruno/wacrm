...
/** A structured error from the CodixIA API envelope (`{ error: { code, message } }`). */
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new WacrmApiError(
        0,
        'network_error',
        `Could not reach CodixIA at ${this.baseUrl}: ${(err as Error).message}`,
      );
    }
