...
User-visible changes in `CodixIA`. Self-hosters: when pulling an update,
...
Assistant**; CodixIA calls the provider directly with that key, so
there's no per-seat AI fee and your conversation data never leaves
your own infrastructure for a CodixIA-run service. The key is stored
...
Multi-user accounts ship. Every CodixIA install is multi-tenant on the
...
- **Public REST API (`/api/v1`) — groundwork.** A scoped, revocable
  **API key** system so you can drive CodixIA from your own scripts and
...
CodixIA is intentionally **single-tenant per WhatsApp number**. RLS on
